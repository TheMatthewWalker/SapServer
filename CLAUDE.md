...
## SAP NCo Spike (`Services/Nco/*`, `Controllers/NcoTestController.cs`)

A parallel, additive proof-of-concept for replacing the SAPFunctions64 COM/STA-thread-pool architecture with the SAP .NET Connector (NCo) — **not a replacement**, the existing `SapConnectionPool`/`SapStaWorker` COM path is untouched and remains what the app actually runs on. This exists to answer, before committing to a full migration: does NCo work at all under .NET 10 from this environment, does it need anything like the STA-thread-per-worker model, and does its resource-cleanup story actually differ from COM's.

- **Why NCo at all**: a standalone .NET Framework 4.8 test proved SAP NCo works with a real SAP system; the immediate blocker trying the same thing directly in this .NET 10 app was a "missing service" error at `RfcDestinationManager.GetDestination(...)` — almost certainly either a missing `System.Configuration.ConfigurationManager` package (NCo probes for it; not present by default on .NET Core) or (more likely the actual cause) no `IDestinationConfiguration` explicitly registered via `RfcDestinationManager.RegisterDestinationConfiguration(...)` before the first `GetDestination` call — see `NcoDestinationConfiguration`, which exists specifically to make that registration explicit rather than relying on file-based config discovery.
- **`RfcRequest`/`RfcResponse`/`RfcRequestBuilder` are already transport-agnostic** — plain POCOs with zero COM dependency. `NcoRfcService.ExecuteAsync` accepts the exact same `RfcRequest` `SapStaWorker.ExecuteRfc` does and returns the same `RfcResponse` shape, so `WhereClauseBuilder`, `SapDelimitedParser`, and every existing `Helpers/*` request builder work unchanged against either transport. Only the executor differs — see `NcoReadTablesHelper` for a small dedicated ZRFC_READ_TABLES builder used by the spike endpoints (mirrors `ProductionHelpers.BuildKgToUnitRequest`'s shape).
- **Connect**: `NcoDestinationConfiguration : IDestinationConfiguration` builds an `RfcConfigParameters` from `SapNcoOptions` (bound from the new `SapNco` config section — see `appsettings.example.json`); `NcoRfcService`'s constructor registers it once (guarded by an `Interlocked` flag — `RegisterDestinationConfiguration` may only be called once per process).
- **Multi-request handling**: no STA thread pool — `RfcDestination` is documented thread-safe with its own internal connection pool (`SapNco:PoolSize`/`MaxPoolSize`), so unlike the COM path there is no dedicated thread to route calls to. `NcoRfcService` bounds concurrency with a `SemaphoreSlim` sized from `MaxPoolSize` and runs each (blocking) NCo call via `Task.Run`. `NcoTestController.ConcurrencyCheck` fires N concurrent `RFC_PING` calls and reports each one's wall-clock time — cross-reference against `NcoRfcService`'s per-call thread-id log lines (same pattern as `SapStaWorker.ProcessItem`'s) to confirm calls genuinely overlap rather than serializing.
- **COM marshal/GC finding (documented in `NcoRfcService`'s class doc comment, restated here since it's the main thing this spike exists to settle)**: SAP NCo is not a COM component, so `SapStaWorker`'s COM-specific cleanup (`Marshal`-level object lifetime, and the mandatory `_sapFunctions.RemoveAll()` every call to stop `SAPFunctions64.Add()` leaking into a COM-side `Functions` collection) has no equivalent here. `IRfcFunction`/`IRfcTable`/`IRfcStructure` are plain managed objects eligible for ordinary GC once they go out of scope, and `RfcRepository.CreateFunction()` has no leaking registry to clean up after. **Not yet verified against a live SAP system or under sustained load** (this sandbox has neither) — treat it as the hypothesis this spike is meant to confirm, not a settled fact.
- **DLLs**: no real `sapnco.dll`/`sapnco_utils.dll` were available to build against here, so `interop-stubs/SapNco.DevStub` supplies a compile-only stand-in for the `SAP.Middleware.Connector` namespace, written from memory of the public NCo 3.0/3.1 API and **not verified against the real assemblies**. `SapServer.csproj` prefers real DLLs the same way it does for `Interop.SAPFunctions64.dll`: drop `sapnco.dll`/`sapnco_utils.dll` into `libs\`, and the matched native binary set (`librfc32.dll`/`librfcum.dll`, `icudt*`/`icuin*`/`icuuc*.dll`, matching VC++ runtime — see the "newer sapnco" DLL discussion) into `libs\sapnco-native\`, and the real API wins over the stub automatically. Expect small signature fixes in `Services/Nco/*.cs` when that happens — the NCo-specific code is deliberately isolated behind `INcoRfcService` so those fixes stay contained.
- **Endpoints are deliberately minimal and not permission-gated yet**: `GET /api/nco-test/material/{material}` (single ZRFC_READ_TABLES → MARA lookup) and `GET /api/nco-test/concurrency-check?count=N`. Both require only a valid JWT (`[Authorize]`, no `CheckPermissionAsync` call) — no `SapDepartmentPermissions` function code has been assigned to this spike.
- **Still to validate on a real dev machine**: actually connecting to a live SAP system (the original "missing service" error, unreproduced here), whether the ExecuteAsync/Task.Run concurrency model holds up under real load, and whether the COM-vs-NCo resource-cleanup finding above actually holds. This dev environment has no SAP GUI and no NCo license either — same constraint the rest of this file already notes for the COM path.
...
| `SapNco:AppServerHost` / `SystemNumber` / `Client` / `User` / `Password` / `Language` | SAP NCo spike connection parameters (see "SAP NCo Spike" above) — separate from `SapPool:ServiceAccount`, not the live COM path |
| `SapNco:PoolSize` / `MaxPoolSize` | NCo's own internal RFC connection pool size for the spike destination, and this app's own concurrency gate (default 4 / 8) — not a thread count |
